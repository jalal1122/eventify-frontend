...
import { mockOrganizers, mockEvents } from "@/lib/dummyData";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to load organizer profile from API, falling back to dummy data", error);
        const mockProfile = mockOrganizers.find(o => o._id === organizerId);
        if (mockProfile) {
          setProfile(mockProfile as any);
          setEvents(mockEvents.filter(e => e.organizerId === organizerId));
        }
      } finally {
        setLoading(false);
      }
